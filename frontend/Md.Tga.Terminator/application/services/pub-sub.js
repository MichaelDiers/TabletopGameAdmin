...
const initialize = (options) => {
  ...
  const client = {
    ...
    publish: async (message) => {
      ...
      const json = `{"processId":"${uuid.v4()}","gameSeriesId":"${gameSeriesId}","gameId":"${gameId}","terminationId":"${terminationId}","winningSideId":"${winningSideId}","reason":"${reason}","rounds":"${rounds}"}`;
